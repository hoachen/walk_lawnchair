...
val fileProviderAuthority: String = "${LauncherSDK.context.packageName}.fileprovider"

fun String.path2Uri(): Uri? = File(this).file2Uri()

fun File.file2Uri(): Uri? = try {
    FileProvider.getUriForFile(LauncherSDK.context, fileProviderAuthority, this)
} catch (e: Exception) {
    e.printStackTrace()
    null
}
